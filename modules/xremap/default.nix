...
let
  ...
  hex = lib.types.strMatching "0x[0-9a-fA-F]{1,4}";
  ...
  # 仮想キーボードの vid/pid を内蔵タッチパッドに揃えるためのフラグ (下記 option 参照)。
  outputDeviceArgs = lib.optionals (cfg.outputDeviceId != null) [
    "--vendor"
    cfg.outputDeviceId.vendor
    "--product"
    cfg.outputDeviceId.product
  ];
in
{
  options.xremap.outputDeviceId = lib.mkOption {
    type = lib.types.nullOr (lib.types.submodule {
      options = {
        vendor = lib.mkOption { type = hex; description = "USB vendor ID (0x 付き 16 進)"; };
        product = lib.mkOption { type = hex; description = "USB product ID (0x 付き 16 進)"; };
      };
    });
    default = null;
    example = { vendor = "0x05ac"; product = "0x027e"; };
    description = ''
      xremap が uinput で作る仮想キーボードの vendor / product ID (`--vendor` / `--product`)。

      libinput の disable-while-typing (DWT、タイプ中のタッチパッド無効化) は
      「タッチパッドとペアになったキーボード」のキー入力しか見ない。xremap は物理
      キーボードを EVIOCGRAB で占有して仮想デバイス (既定 1234:5678) から打ち直すので、
      libinput には物理キーボードのキー入力が届かず DWT が効かなくなる。

      タッチパッドが udev で `ID_INPUT_TOUCHPAD_INTEGRATION=external` と判定される機種
      (USB 接続に見える T2 Mac の apple-bce 等) では、libinput は vid/pid が一致する
      キーボードだけをペアにする (`tp_want_dwt`)。そこで仮想デバイスの vid/pid を
      タッチパッドに揃えて DWT のペアに入れる。
      タッチパッドが internal 判定の機種ではこの設定では足りず、libinput の quirk
      (`AttrKeyboardIntegration=internal`) が要る (docs/xremap.md)。
    '';
  };
    ...
    systemd.user.services.xremap = {
      ...
      Service = {
        ...
        ExecStart = lib.concatStringsSep " " (
          [ "${xremap}/bin/xremap" "--watch=config,device" ]
          ++ outputDeviceArgs
          ++ [ configPath ]
        );
        Restart = "on-failure";
